 ...
 *
 * updated (jstn): single header file style
 *
 * Updated 2/2016 droh:
...
#ifdef CSAPP_IMPLEMENTATION

/* System */
int System(const char *command) {
  int rc = system(command);
  if (!command && rc == 0) {
    // if command is NULL, rc holds a non-zero value if
    // a shell is available, or 0 if no shell is found.
    unix_error("System error: no shell available");
    return rc;
  } else if (!command) {
    return rc; // shell is available, but we have no command to run
  }

  if (rc < 0) {
    unix_error("System error");
  } else if (WIFEXITED(rc) && WEXITSTATUS(rc) != 0) {
    app_error("System error: command exited with non-zero status");
  } else if (WIFSIGNALED(rc) && WTERMSIG(rc) != 0) {
    app_error("System error: command terminated by signal");
  }
...
unsigned int Sleep(unsigned int secs)
{
  ...
  // NOTE: sleep returns 0 when sleep was OK
  // and returns non-zero when it was interrupted
  // so let's just error out when interrupted
  if ((rc = sleep(secs)) != 0)
